...
func (i *Implementation) Delete(ctx context.Context, req *desc.DeleteRequest) (*desc.DeleteResponse, error) {
	if err := validateUsernameRequest(req.GetUsername()); err != nil {
		...
		return &desc.DeleteResponse{}, status.Errorf(codes.InvalidArgument, "validate request: %s", err.Error())
	}

	id, err := i.authService.Delete(ctx, converter.ToUsername(req.GetUsername()))
	if err != nil {
		i.log.Errorf("failed to delete user: %s", err.Error())
		return &desc.DeleteResponse{}, status.Errorf(codes.Internal, "failed to delete user: %s", err.Error())
	}
